...
function DropoffDetailsScreen({ initial, profile, onConfirm, onCancel }) {
  ...
  const confirm = async () => {
    ...
    setLoading(true);
    try {
      const areaTrim = area.trim(), streetTrim = street.trim();
      const loc = await forwardGeocode([streetTrim, areaTrim].filter(Boolean).join("، "));
      const label = [streetTrim, areaTrim].filter(Boolean).join("، ") || areaTrim;
      ...
      onConfirm({ label, loc, notes: noteLines, raw: { forWhom, phone, name, area: areaTrim, street: streetTrim, details } });
    } finally {
      setLoading(false);
    }
  };
                ...
                <div className={errors.area ? "moter-shake" : ""}>
                  ...
                  <div className="relative">
                    ...
                    <input value={area}
                      ...
                      className="w-full border-2 rounded-xl pr-11 pl-4 py-3.5 text-gray-950 focus:outline-none transition-colors" style={{borderColor: errors.area ? "#EF4444" : "var(--bc-b)"}}/>
                    {showSuggestions && (
                  ...
                  {errors.area ? <p className="moter-field-error"><AlertCircle size={13}/> {errors.area}</p>
                    : <p className="text-gray-400 text-xs mt-1.5">اسم الحي أو المنطقة</p>}
                </div>
              </div>
