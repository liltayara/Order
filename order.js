...
function WelcomeScreen({ onSelect }) {
  const { t, darkMode, setDarkMode } = useApp();
  return (
    <div className="min-h-screen bg-white flex flex-col relative" dir="rtl">
      <button onClick={()=>setDarkMode(v=>!v)} aria-label={darkMode ? "تفعيل الوضع النهاري" : "تفعيل الوضع الليلي"} aria-pressed={darkMode}
        className="absolute top-5 left-5 z-10 w-11 h-11 rounded-2xl flex items-center justify-center bg-white/10 border border-white/15 text-white backdrop-blur-sm active:scale-90 transition-transform">
...
function AppShell() {
  ...
  const ctxValue = { darkMode: appSettings.darkMode, setDarkMode, t, notifications, markAllRead, orders, addOrder, updateOrder, showToast, pushNotification, account: sessionAccount, updateProfile };
