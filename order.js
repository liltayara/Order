...
function CForm({ setView, form, setForm }) {
  const [errors, setErrors] = useState({});
  const [picker, setPicker] = useState(null); // "dropoff" | null — الاستلام صار تلقائياً، لا يُفتح له مُنتقي بعد الآن
  const [shakeKey, setShakeKey] = useState(0);
  ...
  const confirmLocation = (loc) => {
    setForm({ ...form, dropoff: loc.label, dropoffLoc: { lat: loc.lat, lng: loc.lng } });
    setPicker(null);
  };
  ...
  return (
    <div>
      <div className="bg-gray-950 text-white p-5 pt-14">
        ...
        <p className="text-gray-400 text-sm mt-1">موقعك الحالي هو نقطة الاستلام — حدّد نقطة التسليم فقط</p>
      </div>
