...
// ══════════════════════════════════════════════════════════════════════════════
// بصمة الجهاز — لمنع إنشاء حساب جديد من نفس الجهاز بعد حظره، وليس فقط حظر
// الحساب نفسه. تُشتق من خصائص المتصفح/الجهاز شبه الثابتة، وتُخزَّن في
// localStorage و IndexedDB معاً (لو مُسح أحدهما يبقى الآخر يُرجعها). ليست
// معرّفاً مضموناً 100% — تتغيّر لو غيّر الشخص متصفحه بالكامل أو مسح كل بيانات
// الجهاز يدوياً — لكنها ترفع حاجز إعادة التسجيل بشكل كبير عن مجرد حساب جديد.
// ══════════════════════════════════════════════════════════════════════════════
const DEVICE_ID_KEY = "order_device_id_v1";
...
function canvasFingerprint() {
  try {
    ...
    ctx.fillStyle = "#069";
    ctx.fillText("order-device-فحص-🚀", 2, 2);
    return c.toDataURL();
...
function idbGet(key) {
  return new Promise((resolve) => {
    try {
      const req = indexedDB.open("order_device_store", 1);
      req.onupgradeneeded = () => { try { req.result.createObjectStore("kv"); } catch (e) {} };
      req.onerror = () => resolve(null);
      req.onsuccess = () => {
        try {
          const tx = req.result.transaction("kv", "readonly").objectStore("kv").get(key);
...
function idbSet(key, value) {
  return new Promise((resolve) => {
    try {
      ...
      req.onsuccess = () => {
        try {
          const tx = req.result.transaction("kv", "readwrite").objectStore("kv").put(value, key);
...
// معرّف الجهاز: نحترم أي معرّف مخزَّن مسبقاً (حتى لو تغيّرت البصمة الخام بعد
// تحديث متصفح مثلاً) طالما لم تُمسح بيانات التخزين، وإلا نولّد واحداً جديداً.
async function getDeviceId() {
  if (cachedDeviceId) return cachedDeviceId;
  let existing = null;
  try { existing = localStorage.getItem(DEVICE_ID_KEY); } catch (e) {}
  if (!existing) existing = await idbGet(DEVICE_ID_KEY);
  if (existing) {
    cachedDeviceId = existing;
    try { localStorage.setItem(DEVICE_ID_KEY, existing); } catch (e) {}
    idbSet(DEVICE_ID_KEY, existing);
    return existing;
  }
  const raw = [
    navigator.userAgent, navigator.language, navigator.hardwareConcurrency || "",
    navigator.maxTouchPoints || "", screen.width, screen.height, screen.colorDepth,
    Intl.DateTimeFormat().resolvedOptions().timeZone, canvasFingerprint(),
  ].join("|");
  const id = await sha256Hex(raw);
  cachedDeviceId = id;
  try { localStorage.setItem(DEVICE_ID_KEY, id); } catch (e) {}
  idbSet(DEVICE_ID_KEY, id);
  return id;
}
...
const bannedDevicesCol = fbDb.collection("bannedDevices");
...
const fbApi = {
  ...
  async continueWithUsername({ role, username, password }) {
    ...
    // فحص حظر الجهاز أولاً — قبل أي محاولة مصادقة، فيُمنع الجهاز المحظور من
    // إنشاء حساب جديد أو حتى الدخول بحساب موجود عليه. فشل الفحص نفسه (لا
    // اتصال مثلاً) لا يمنع المتابعة — لا نعاقب مستخدماً بريئاً بسبب عطل شبكة.
    try {
      const devSnap = await bannedDevicesCol.doc(deviceId).get();
      if (devSnap.exists) throw new Error("تم حظر هذا الجهاز من استخدام التطبيق");
    } catch (e) {
      if (e.message === "تم حظر هذا الجهاز من استخدام التطبيق") throw e;
    }
    ...
    if (isNew) {
      const profile = blankProfile({ uid, role, authMethod: "username", authEmail, extra: { username: uname, deviceIds: [deviceId] } });
    ...
    try { await usersCol.doc(uid).set({ deviceIds: firebase.firestore.FieldValue.arrayUnion(deviceId) }, { merge: true }); } catch (e) {}
  ...
  async continueWithGoogle({ role }) {
    ...
    if (isNew) {
      const profile = blankProfile({
        ...
        extra: { googleUid, email: googleEmail || null, name: googleName, photoURL: googlePhoto, deviceIds: [deviceId] },
      });
      try { await usersCol.doc(uid).set(profile); }
      catch (e) { throw new Error(FIRESTORE_SETUP_ERR); }
      return { ...profile, id: uid, createdAt: new Date().toISOString() };
...
function AppShell() {
  ...
  useEffect(() => {
    ...
    const unsubAuth = fbAuth.onAuthStateChanged((user) => {
      ...
      unsubDoc = usersCol.doc(user.uid).onSnapshot(async (snap) => {
        if (snap.exists) {
          const acc = { ...snap.data(), id: user.uid };
          let deviceBanned = false;
          try {
            const myDeviceId = await getDeviceId();
            const devSnap = await bannedDevicesCol.doc(myDeviceId).get();
            deviceBanned = devSnap.exists;
          } catch (e) {}
          if (acc.banned || deviceBanned) {
