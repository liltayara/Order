...
function DropoffDetailsScreen({ initial, onConfirm, onCancel }) {
  ...
  const validate = () => {
    ...
    else if (phoneDigits.length !== 9) errs.phone = "رقم الهاتف غير صحيح";
  ...
  const confirm = async () => {
    ...
    try {
      ...
      const noteLines = [
        `هاتف المستلم: ${toSudanE164(phone)}`,
        name.trim() ? `الاسم: ${name.trim()}` : null,
        details.trim() ? `تفاصيل إضافية: ${details.trim()}` : null,
      ].filter(Boolean).join("\n");
      onConfirm({ label, loc, notes: noteLines, raw: { phone, name, area: areaTrim, street: streetTrim, details } });
    } finally {
      setLoading(false);
    }
  };
