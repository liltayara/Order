...
function DropoffDetailsScreen({ initial, profile, onConfirm, onCancel }) {
  ...
  const areaSuggestions = useMemo(() => {
    const q = area.trim();
    if (!q) return [];
    return NEIGHBORHOODS.filter(n => n.name.includes(q)).slice(0, 6);
  }, [area]);
  const showSuggestions = areaFocused && area.trim().length > 0;
  const matchedArea = useMemo(() => NEIGHBORHOODS.find(n => n.name === area.trim()) || null, [area]);
  ...
  const validate = () => {
    ...
    if (!area.trim()) errs.area = "المنطقة مطلوبة";
    else if (!matchedArea) errs.area = "اسم المدينة أو المنطقة خطأ — اختر من القائمة المقترحة";
    setErrors(errs);
    if (Object.keys(errs).length) setShakeKey(k => k + 1);
    return Object.keys(errs).length === 0;
  };
  ...
  const confirm = async () => {
    ...
    try {
      ...
      const loc = { lat: matchedArea.lat, lng: matchedArea.lng };
      const label = [streetTrim, matchedArea.name].filter(Boolean).join("، ");
      ...
      onConfirm({ label, loc, notes: noteLines, raw: { forWhom, phone, name, area: matchedArea.name, street: streetTrim, details } });
    } finally {
      setLoading(false);
    }
  };
                ...
                <div className={errors.area ? "moter-shake" : ""}>
                  ...
                  <div className="relative">
                    ...
                    <input value={area}
                      ...
                      aria-label="المنطقة" autoComplete="off"
                      className="w-full border-2 rounded-xl pr-11 pl-11 py-3.5 text-gray-950 focus:outline-none transition-colors" style={{borderColor: errors.area ? "#EF4444" : "var(--bc-b)"}}/>
                    {matchedArea && <CheckCircle size={18} className="text-green-500 absolute left-3.5 top-1/2 -translate-y-1/2"/>}
                    {showSuggestions && (
                      <div className="absolute z-10 top-full mt-1.5 left-0 right-0 bg-white border-2 rounded-xl overflow-hidden shadow-lg" style={{borderColor:"var(--bc-a)"}}>
                        {areaSuggestions.length > 0 ? areaSuggestions.map(n => (
                          ...
                          <div className="px-4 py-3 flex items-center gap-2.5">
                            ...
                            <span className="text-sm text-gray-400">لا توجد مدينة أو منطقة بهذا الاسم</span>
                          </div>
                        )}
                      </div>
                    )}
